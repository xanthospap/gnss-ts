...
public:
        ...
        if (t >= j->start()) {
          ...
          */
          ...
          \"make_model()\""; std::cerr<<"\nHere are the variables:";
                                  ...
                                  std::cerr<<"\nFP Exceptions Set:";
                                  std::cerr<<"\n\tFE_DIVBYZERO:
          "<<std::fetestexcept(FE_DIVBYZERO); std::cerr<<"\n\tFE_INEXACT  :
          "<<std::fetestexcept(FE_INEXACT); std::cerr<<"\n\tFE_INVALID  :
          "<<std::fetestexcept(FE_INVALID); std::cerr<<"\n\tFE_OVERFLOW :
          "<<std::fetestexcept(FE_OVERFLOW); std::cerr<<"\n\tFE_UNDERFLOW:
          "<<std::fetestexcept(FE_UNDERFLOW); std::exit(1);
                              }
          #endif
          */
        }
  ...
  make_model(datetime<T> start, datetime<T> stop,
    ...
    std::size_t size_hint =
        (std::size_t)((stop.as_mjd() - start.as_mjd()) / tinterval.as_mjd()) +
        1;
  ...
  ///       (not assigned).
  void assign_solution_vector(const Eigen::VectorXd &x_estim,
                              const Eigen::MatrixXd *Q = nullptr) {
    if (!this->is_linear()) {
    ...
    for (auto j = m_harmonics.begin(); j != m_harmonics.end(); ++j) {
      j->in_phase() = x_estim(idx);
      ++idx;
      j->out_of_phase() = x_estim(idx);
      ++idx;
    }
    ...
    if (Q) {
      ...
      idx = 0;
      m_x0_stddev = (*Q)(idx, idx);
      ++idx;
      m_vx_stddev = (*Q)(idx, idx);
      ++idx;
      for (auto j = m_harmonics.begin(); j != m_harmonics.end(); ++j) {
        j->in_phase_stddev() = (*Q)(idx, idx);
        ++idx;
        j->out_phase_stddev() = (*Q)(idx, idx);
        ++idx;
      }
      for (auto j = m_jumps.begin(); j != m_jumps.end(); ++j) {
        j->stddev() = (*Q)(idx, idx);
  ...
  void dump(std::ostream &os) const {
    ...
    for (auto j = m_earthqs.begin(); j != m_earthqs.end(); ++j) {
      ...
      os << "\nTime of earthquake: " << strftime_ymd_hms<T>(j->start())
         << " (MJD " << j->start().as_mjd() << ")";
      if (p == 1) {
        os << "\n\tOffset: " << j->a1();
      }
  ...
  ///                         sqrt(A^2 + B^2).
  void filter_parameters(double min_abs_jump = 1e-3,
                         double min_abs_harm = 1e-3) {
  ...
  void assign_row(Eigen::MatrixXd &A, Eigen::VectorXd &b, const datetime<T> &t,
    ...
    for (auto j = m_earthqs.cbegin(); j != m_earthqs.cend(); ++j) {
      ...
      if (t >= j->start()) {
        ...
        for (std::size_t i = 0; i < idrv; i++)
          A(row, col + i) = w * derivs[i];
        l += j->value_at(t);
      } else {
        for (std::size_t i = 0; i < idrv; i++)
          A(row, col + i) = 0e0;
  ...
  std::vector<md_harmonics<T>> harmonics() const noexcept {
    return m_harmonics;
  }

  /// Return a copy of the harmonics vector of the model.
  std::vector<md_harmonics<T>> &harmonics() noexcept { return m_harmonics; }

  /// Return a copy of the earthquake events vector of the model.
  std::vector<md_earthquake<T>> earthquakes() const noexcept {
    return m_earthqs;
  }

  /// Clear (i.e remove) all earthquakes
  void clear_earthquakes() noexcept { m_earthqs.clear(); }

  /// Remove a harmonic term given a period
  int remove_harmonic_with_period(double per) noexcept {
    std::size_t original_size = m_harmonics.size();
    m_harmonics.erase(std::remove(m_harmonics.begin(), m_harmonics.end(),
                                  [per](const md_harmonics<T> &h) {
                                    return h.period() == per;
                                  }),
                      m_harmonics.end());
  ...
  harmonic_with_period(double per) const noexcept {
    return std::find_if(
        m_harmonics.cbegin(), m_harmonics.cend(),
...
template <class T>
void models_to_json(std::ostream &os, const ts_model<T> &x,
