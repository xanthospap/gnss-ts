...
public:
    ...
    explicit
    flag(std::initializer_list<FlagEnum>&& fs)
    : _f{}
    ...
    void
    set(FlagEnum f) noexcept
    { _f |= static_cast<ft>(f); }
    
    /// Clear a FlagEnum.
    /// @param[in] f  Clear (i.e. "switch off") this FlagEnum.
    void
    clear(FlagEnum f) noexcept
    { _f &= ~(static_cast<ft>(f)); }
    ...
    bool
    check(FlagEnum f) const noexcept
    { return _f & static_cast<ft>(f); }
    ...
    ///         false otherwise.
    bool
    operator==(flag f) const noexcept
    { return _f == f._f; }
