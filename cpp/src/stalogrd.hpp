...
ngpt::datetime<T> strptime_log(const char *str, char **stop = nullptr) {
  ...
  if (stop)
    *stop = end - 1;
  return ngpt::datetime<T>{year{ints[0]},         month{ints[1]},
...
public:
  ...
  template <typename T> std::vector<ngpt::datetime<T>> receiver_changes() {
    ...
    // Information'
    while (m_ifs.getline(line, MAX_CHARS) &&
           strcmp(receiver_info_block, line)) {
      if (++dummy >= MAX_LINES) {
  ...
  template <typename T> std::vector<ngpt::datetime<T>> antenna_changes() {
    std::size_t dummy{0};
    constexpr std::size_t MAX_CHARS{256}, MAX_LINES{10000};
...
private:
  ...
  template <typename T>
  int read_receiver_block(std::string &receiver_type, ngpt::datetime<T> &start,
                          ngpt::datetime<T> &stop) {
  ...
  template <typename T>
  int read_antenna_block(std::string &antenna_type, ngpt::datetime<T> &start,
                         ngpt::datetime<T> &stop) {
    constexpr std::size_t MAX_CHARS{256}, MAX_LINES{1000};
    char line[MAX_CHARS];
