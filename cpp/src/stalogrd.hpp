...
int
str_is_empty(const char* str) noexcept
{
    while (*str) {
...
/// Resolve a datetime from a string of type: 'CCYY-MM-DDThh:mmZ' or 'CCYY-MM-DD'
///
...
public:
        ...
        receiver_changes()
    {
        ...
        while ( m_ifs.getline(line, MAX_CHARS) && strcmp(receiver_info_block, line) ) {
            if (++dummy >= MAX_LINES) {
                throw std::invalid_argument
                    ("receiver_changes: Cannot find block \'GNSS Receiver Information\'!");
            }
        }
...
private:
    ...
    /// If the first line read is something different, an error is signaled.
    /// It then will read all recorded information and return the important
    /// fields. The block should be formatted exactly as described at the IGS
    /// specifications (see ftp://igs.org/pub/station/general/sitelog_instr.txt).
    /// The function (if successeful), will leave the stream after having read
    /// all info of the block, including any 'Additional Information'. That
    /// means that the next line to be read, should be an empty line.
    ///
    ...
    ///                           start is set to the min datetime<T>. The same
    ///                           goes for an empty string.
    /// @param[out] stop          Stop time of this block's validity (line
    ///                           'Date Removed). If this is set to the string
    ///                           'CCYY-MM-DDThh:mmZ' inside the block, then
    ///                           stop is set to the max datetime<T>. The same
    ///                           goes for an empty string.
        ...
        read_receiver_block(std::string& receiver_type, ngpt::datetime<T>& start, ngpt::datetime<T>& stop)
    {
        ...
        if ( line[2] == 'x' ) return 1; // line 3.x; exit
        ...
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Date Installed           :", line+5, 26) ) {
            ...
            if ( igs_log_details::str_is_empty(line+31) ) {
                std::cerr<<"\n[DEBUG] read_receiver_block: \'Date Installed\' record is empty; setting to min date.";
        ...
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Date Removed             :", line+5, 26) ) {
            ...
            if ( igs_log_details::str_is_empty(line+31) ) {
                std::cerr<<"\n[DEBUG] read_receiver_block: \'Date Removed\' record is empty; setting to max date.";
        ...
        while ( new_line && m_ifs.getline(line, MAX_CHARS) ) {
            if ( !igs_log_details::str_is_empty(line) ) {
                new_line = true;
                pos      = m_ifs.tellg();
            } else {
                new_line = false;
    ...
    template<typename T>
        int
        read_antenna_block(std::string& antenna_type, ngpt::datetime<T>& start, ngpt::datetime<T>& stop)
    {
        constexpr std::size_t MAX_CHARS {256},
                              MAX_LINES {1000};
        char line[MAX_CHARS];
        ...
        if ( line[0] != '4' || line[1] != '.' ) return -1;
        if ( line[2] == 'x' ) return 1; // line 4.x; exit

        if ( strncmp("Antenna Type             :", line+5, 26) ) return -1;
        antenna_type.assign(line+31, 20);
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Serial Number            :", line+5, 26) )
            return -2;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Antenna Reference Point  :", line+5, 26) )
            return -3;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Marker->ARP Up Ecc. (m)  :", line+5, 26) )
            return -4;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Marker->ARP North Ecc(m) :", line+5, 26) )
            return -5;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Marker->ARP East Ecc(m)  :", line+5, 26) )
            return -6;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Alignment from True N    :", line+5, 26) )
            return -7;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Antenna Radome Type      :", line+5, 26) )
            return -8;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Radome Serial Number     :", line+5, 26) )
            return -9;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Antenna Cable Type       :", line+5, 26) )
            return -10;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Antenna Cable Length     :", line+5, 26) )
            return -11;
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Date Installed           :", line+5, 26) ) {
            return -12;
        } else {
            if ( igs_log_details::str_is_empty(line+31) ) {
                std::cerr<<"\n[DEBUG] read_antenna_block: \'Date Installed\' record is empty; setting to min date.";
                start = ngpt::datetime<T>::min();
            } else {
                int pos = 31;
                // go to the start of the datetime string
        ...
        if ( !m_ifs.getline(line, MAX_CHARS) || strncmp("Date Removed             :", line+5, 26) ) {
            ...
            if ( igs_log_details::str_is_empty(line+31) ) {
                std::cerr<<"\n[DEBUG] read_antenna_block: \'Date Removed\' record is empty; setting to max date.";
                stop = ngpt::datetime<T>::max();
            } else {
                int pos = 31;
                // go to the start of the datetime string
