...
bool is_event(ngpt::flag<ts_event> f) noexcept {
  return f.check(ts_event::jump) || f.check(ts_event::velocity_change) ||
...
/// @see genflags.hpp
///
template <class T, typename = std::enable_if_t<T::is_of_sec_type>> class event {
public:
  ...
  bool operator==(const event<T> &e) const noexcept {
...
public:
  ...
  explicit event_list(const std::vector<event<T>> &vec) noexcept
      : m_events(vec) {}
  ...
  limit_copy(ngpt::datetime<T> start = ngpt::datetime<T>::min(),
             ngpt::datetime<T> stop = ngpt::datetime<T>::max()) const noexcept {
    ...
    if (start == ngpt::datetime<T>::min())
      start = m_events[0].epoch();
    if (stop == ngpt::datetime<T>::max())
  ...
  void apply(ngpt::flag<ts_event> f, const ngpt::datetime<T> &t,
             std::string info = std::string{""}) noexcept {
    if (is_event(f)) {
  ...
  void apply(const event<T> &e) noexcept { this->sorted_insert(e); }
  ...
  /// @param[in] info An information string (optional)
  void apply(ts_event f, const ngpt::datetime<T> &t,
             std::string info = std::string{""}) noexcept {
  ...
  void
  split_event_list(std::vector<ngpt::datetime<T>> &jumps,
                   std::vector<ngpt::datetime<T>> &vel_changes,
                   std::vector<ngpt::datetime<T>> &earthquakes) const noexcept {
    jumps.clear();
    vel_changes.clear();
    earthquakes.clear();
  ...
  typename std::vector<event<T>>::const_iterator it_cbegin() const noexcept {
    return m_events.cbegin();
  }

  /// Get a const_iterator to end (i.e. one past the end) the of the
  /// m_events vector.
  typename std::vector<event<T>>::const_iterator it_cend() const noexcept {
    return m_events.cend();
  }

  /// Get an iterator to the begining of the m_events vector.
  typename std::vector<event<T>>::const_iterator it_begin() noexcept {
    return m_events.begin();
  }

  /// Get an iterator to end (i.e. one past the end) the of the
  /// m_events vector.
  typename std::vector<event<T>>::iterator it_end() noexcept {
    return m_events.end();
  }
  ...
  void filter_earthquake_sequences(ngpt::datetime_interval<T> dt) {
    if (!m_events.size())
      return;
    ...
    auto it =
        std::find_if(m_events.begin(), m_events.end(), [](const event<T> &e) {
  ...
  event<T> event_at(const ngpt::datetime<T> &t) const {
  ...
  std::vector<event<T>> &get_the_vector() noexcept { return this->m_events; }
...
private:
  ...
  ///
  bool sorted_insert(const event<T> &new_event) {
  ...
  ///            be sorted (chronologicaly).
  ///
  bool sorted_insert(ts_event e, const epoch &t,
