...
template <class T>
ngpt::ts_model<T> identify_harmonics(ngpt::timeseries<T, ngpt::pt_marker> &ts,
                                     ngpt::ts_model<T> &model, double Ut = 1e-3,
                                     double min_amplitude = 0e0) {
  ...
  px = mempool;
  py = mempool + nout;
  auto nmodel{model}, amodel{model};
  double nstddev, astddev, factor;
  auto nts{ts};
  // first fit.
  ...
  while (dummy_it < 10) {
    ...
    std::cout << "\n\tDominant frequency in time-series: " << px[jmax]
              << " (at: " << jmax << ")"
              << "; this is a period of " << 1e0 / px[jmax]
              << " days -- posibility: " << prob << " --";
    ...
    if ((factor - 1e0) > Ut && ampl > min_amplitude) {
      ...
      astddev = nstddev;
      nmodel = amodel;
      std::cout << "\n\tFrequency added to the model; factor was: "
                << (factor - 1e0);
    } else {
      break;
    }
  ...
  if (dummy_it >= 50) {
    std::cerr << "\n[ERROR] Harmonic Analysis is corrupt! More than 50 "
...
///                      variables will be indentical to the a-priori model.
///                      Yet, this model will only include a subset of the
///                      earthquakes; the ones with statisticaly significant
///                      offset values.
template <class T>
ngpt::ts_model<T> filter_earthquakes(ngpt::timeseries<T, ngpt::pt_marker> &ts,
                                     ngpt::ts_model<T> &model,
                                     double Ut = 1e-3) {
#ifdef DEBUG
  std::cout << "\n[DEBUG] Filtering earthquake events based on their "
  ...
  std::sort(
      erthqk_vec.begin(), erthqk_vec.end(),
      [](const ngpt::md_earthquake<T> &a, const ngpt::md_earthquake<T> &b) {
  ...
  for (auto i : erthqk_vec) {
    std::cout << "\n\tEpoch: " << ngpt::strftime_ymd_hms(i.start())
              << " offset: " << i.offset();
  }
#endif

  // Initialize a new model (identical to the a-priori) with no earthquakes.
  ngpt::ts_model<ngpt::milliseconds> amodel{model}, nmodel{model};
  ...
  typename std::vector<ngpt::md_earthquake<T>>::iterator it =
                                                             erthqk_vec.begin(),
                                                         it_end =
                                                             erthqk_vec.end();
  double factor;
  ...
  for (; it != it_end; ++it) {
...
#ifdef DEBUG
    std::cout << "\n\tTesting earthquake: "
              << ngpt::strftime_ymd_hms(it->start())
              << ", Jump Value:" << it->offset();
#endif
    nmodel.add_earthquake(*it);
    ts.qr_ls_solve(nmodel, stddev_n, 1e-3, false, false);
#ifdef DEBUG
    std::cout << "\n\t\tResulting std. dev: " << stddev_n
              << " (alpha model std.dev: " << stddev_a << ")";
#endif
    ...
    if ((factor - 1e0) > Ut && last_eq.abs_offset() > 1e-3) {
      ...
      std::cout << "\n\t[DEBUG] Earthquake at "
                << ngpt::strftime_ymd_hms(it->start())
                << " removed from the model; factor is " << factor
                << ", value is: " << it->a1();
      nmodel = amodel;
      stddev_n = stddev_a;
    }
...
try_earthquakes(ngpt::timeseries<T, ngpt::pt_marker> &ts,
                ngpt::ts_model<T> &model,
                double min_mag = 0e0 /*, const event_list<T>* evns=nullptr*/,
                double Ut = 1e-3) {
  ...
  if (min_mag && evns) {
    erthqk_vec.erase(std::remove_if(erthqk_vec.begin(), erthqk_vec.end(),
          [min_mag, &evns](const ngpt::md_earthquake<T>& e){return
  e.magnitude(*evns) < min_mag;}), erthqk_vec.end());
  }
  } catch (std::exception& e) {
  ...
  */
  if (min_mag) {
    std::cerr << "\n[WARNING] Skipping minimum magnitude for now .... "
                 "(try_earthquakes)";
  }

  // Initialize a new model (identical to the a-priori) with earthquakes.
  ngpt::ts_model<ngpt::milliseconds> amodel{model}, nmodel{model},
      logmdl{model}, expmdl{model};
