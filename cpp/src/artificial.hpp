...
///                (stop - start) / dt in days.
template <class T>
std::size_t pts_in_interval(datetime<T> start, datetime<T> stop,
                            datetime_interval<T> dt) {
  assert(stop >= start);
  double max_in_day = static_cast<double>(T::max_in_day);
  auto diff = stop.delta_date(start);
  double dpts = static_cast<double>(diff.days()) + diff.secs() / max_in_day;
  double ddt = static_cast<double>(dt.days()) + dt.secs() / max_in_day;
  assert(diff > 0 && ddt > 0);
...
/// @parameter[in] model  A model to characterize the data-points; every
/// data-points
///                       at each instance in the range start:stop:dt , is
///                       computed via this model.
...
timeseries<T, F> synthetic_ts(datetime<T> start, datetime<T> stop,
                              datetime_interval<T> dt, const ts_model<T> &model,
                              double mean = 0e0, double stddev = 0.5e0) {
...
///
template <class T, class F>
timeseries<T, F> synthetic_ts(std::vector<datetime<T>> &epochs,
                              const ts_model<T> &model, double mean = 0e0,
                              double stddev = 0.5e0) {
  std::random_device rd;
  std::mt19937 gen{rd()};
  std::normal_distribution<double> d(mean, stddev);
