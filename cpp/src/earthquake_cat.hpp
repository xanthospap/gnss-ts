...
///               'YYYY OOO DD HH MM SS.S'
template <typename T> std::string strfdt_as_noa(const ngpt::datetime<T> &t) {
  using ngpt::_d2s_;
  using ngpt::_i2s_;
...
struct earthquake {
  ...
  earthquake() noexcept
      : m_epoch{}, m_lon{0}, m_lat{0}, m_depth{0}, m_magnitude{0} {}
  ...
  explicit earthquake(const ngpt::datetime<T> &d, double lat, double lon,
      ...
      : m_epoch{d}, m_lon{lon}, m_lat{lat}, m_depth{dpth}, m_magnitude{mag} {}
...
///       meters.
template <typename T>
earthquake<T> resolve_noa_earthquake_line(const char *line) {
  float info[4];
  ...
  earthquake<T> eqt{eph, ngpt::deg2rad<double>(info[0]),
                    deg2rad<double>(info[1]), info[2] / 1000e0, info[3]};
  return eqt;
}
...
public:
  ...
  bool read_next_earthquake(earthquake<T> &eq) {
    ...
    eq = resolve_noa_earthquake_line<T>(line);
    return true;
  }
  ...
  ///         (if found) and return true. False will be returned if the stream
  ///         fails or EOF is encountered without finding a record in the file
  ///         that happens at an equal or later time than the given epoch.
  bool goto_epoch(ngpt::datetime<T> eph) {
    earthquake<T> eq;
    std::ifstream::pos_type _pos = m_end_of_header;
    if (m_ifs.tellg() != m_end_of_header)
      this->rewind();
    while (read_next_earthquake(eq)) {
      if (eq.epoch() >= eph)
        break;
      else
        _pos = m_ifs.tellg();
