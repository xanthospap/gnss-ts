...
template<typename T>
  earthquake<T>
resolve_noa_earthquake_line(const char* line)
{
  float info[4];
  ...
  for (int i=0; i<4; ++i) {
    ...
    if (errno == ERANGE || start == end) {
      ...
      throw std::invalid_argument
        ("resolve_noa_earthquake_line: Invalid line: \""+std::string(line)+"\" (argument #"+
...
public:
  ...
  /// Navigate to a position in the catalogue file, such that the next earthquake
  /// to be read in will be at an epoch >= to the given epoch
  ///
  ...
  bool
  goto_epoch(ngpt::datetime<T> eph)
  {
    earthquake<T> eq;
    std::ifstream::pos_type _pos = m_end_of_header;
    if (m_ifs.tellg()!=m_end_of_header) this->rewind();
    while (read_next_earthquake(eq)) {
      if (eq.epoch() >= eph) break;
      else _pos = m_ifs.tellg();
    }
