...
/// * an event_list and
/// * a coordinate_type
template <class T, typename = std::enable_if_t<T::is_of_sec_type>> class crdts {
public:
  ...
  crdts(timeseries<T, pt_marker> t1, timeseries<T, pt_marker> t2,
        timeseries<T, pt_marker> t3) {
    m_x = std::move(t1);
    m_y = std::move(t2);
    m_z = std::move(t3);
    assert(m_x.epoch_ptr() == m_y.epoch_ptr() &&
           m_y.epoch_ptr() == m_z.epoch_ptr());
  ...
  crdts(const crdts &ts, std::size_t start = 0, std::size_t end = 0)
      : m_name{ts.m_name}, m_epochs{}, m_x{ts.m_x, start, end}, m_y{ts.m_y,
                                                                    start, end},
        m_z{ts.m_z, start, end}, m_events{ts.m_events}, m_ctype{ts.m_ctype} {
  ...
  /// Move constructor
  crdts(crdts &&ts)
      : m_name{std::move(ts.m_name)}, m_epochs{std::move(ts.m_epochs)},
        m_x{std::move(ts.m_x)}, m_y{std::move(ts.m_y)}, m_z{std::move(ts.m_z)},
        m_events{std::move(ts.m_events)}, m_ctype{std::move(ts.m_ctype)} {
    set_epoch_ptr();
  }
  ...
  void add(const epoch &t, double x, double y, double z, double sx = 1e-3,
           double sy = 1e-3, double sz = 1e-3, tflag fx = tflag{},
           tflag fy = tflag{}, tflag fz = tflag{}) {
  ...
  datetime<T> first_epoch() const noexcept { return m_epochs[0]; }
  ...
  event_list<T> events() const noexcept { return m_events; }

  /// Return the event list
  event_list<T> &events() noexcept { return m_events; }

  timeseries<T, ngpt::pt_marker> &x_component() noexcept { return m_x; }

  timeseries<T, ngpt::pt_marker> &y_component() noexcept { return m_y; }

  timeseries<T, ngpt::pt_marker> &z_component() noexcept { return m_z; }

  /// Return the mean epoch
  epoch mean_epoch() const noexcept { return m_x.central_epoch(); }
  ...
  /// augmented to hold the earthquakes of interest. For an earthquake to be
  /// of interest, it has to:
  /// 1. its epicenter be located less than 300km away
  /// 2. its magnitude be more than min_mag
  /// 3. the equation magnitude >= c1 + c2 * std::log10(distance in km) must
  /// hold
  ///
  /// @todo Should i also mark the ts records??
  std::size_t apply_earthquake_catalogue(earthquake_catalogue<T> &catalogue,
                                         double min_mag = 0e0,
                                         double c1 = 0.5e0, double c2 = 22e0) {
    datetime<T> start{this->first_epoch()}, stop{this->last_epoch()};
    earthquake<T> eq;
    ...
    if (!catalogue.goto_epoch(start)) {
      catalogue.clear_stream_state();
      return eq_applied;
    }
    while (catalogue.read_next_earthquake(eq) && eq.epoch() <= stop) {
      ...
      if (((distance < 300e0) && (eq.magnitude() >= min_mag)) &&
          (eq.magnitude() >= c1 + c2 * std::log10(distance))) {
        m_events.apply(ts_event::earthquake, eq.epoch(), eq.to_string());
        ++eq_applied;
#ifdef DEBUG
        std::cout << "\n[DEBUG] Adding earthquake at " << eq.epoch().stringify()
                  << " (epicentre distance: " << distance << " km), of size "
                  << eq.magnitude() << "M.";
#endif
      }
    }
  ...
  void cartesian2topocentric() noexcept {
    ...
    ngpt::core::car2top_cov_matrix(sinf * sinf, sinl * sinl, cosf * cosf,
                                   cosl * cosl, cf_s);

#ifdef DEBUG
    assert(m_x.data_pts() == m_y.data_pts() &&
           m_y.data_pts() == m_z.data_pts() && m_x.data_pts() == size());
#endif
    for (std::size_t i = 0; i < size(); ++i) {
      ...
      px.sigma() =
          std::sqrt(cf_s[0] * xsigma2 + cf_s[1] * ysigma2 + cf_s[2] * zsigma2);
      py.sigma() =
          std::sqrt(cf_s[3] * xsigma2 + cf_s[4] * ysigma2 + cf_s[5] * zsigma2);
      pz.sigma() =
          std::sqrt(cf_s[6] * xsigma2 + cf_s[7] * ysigma2 + cf_s[8] * zsigma2);
      m_x[i] = px;
      m_y[i] = py;
      m_z[i] = pz;
    }
  ...
  void clear_event_list(ngpt::datetime_interval<T> dt, std::size_t npts = 10) {
    if (!m_events.size())
      return;
    ...
    for (; it != iend; ++it) {
      ...
      if ((t2.delta_date(t1) < dt) && (e1 != ts_event::velocity_change &&
                                       e2 != ts_event::velocity_change)) {
        std::cout << "\n\t[DEBUG] Event at " << strftime_ymd_hms(t1)
                  << " not added because next is too close ("
                  << strftime_ymd_hms(t2) << ")";
        ;
        ...
        if ((idx2 - idx1 < npts) && (e1 != ts_event::velocity_change &&
                                     e2 != ts_event::velocity_change)) {
          std::cout << "\n\t[DEBUG] Event at " << strftime_ymd_hms(t1)
                    << " not added because intermediate points are too few, "
                    << idx2 - idx1 << " (" << strftime_ymd_hms(t2) << ")";
          ;
        } else {
          std::cout << "\n\t[DEBUG] Event at " << strftime_ymd_hms(t1)
                    << " added";
  ...
  void apply_stalog_file(const char *log_file) {
  ...
  auto qr_fit(ngpt::ts_model<T> &xmodel, ngpt::ts_model<T> &ymodel,
              ngpt::ts_model<T> &zmodel) {
  ...
  std::tuple<entry, entry, entry> ddata(std::size_t i) const {
  ...
  void test_period() {
    ...
    std::cout << "\nDominant frequency in time-series: " << py[jmax]
              << " (at: " << jmax << ")";
    std::cout << "\nAs yearly fraction: 1/" << days_in_year * px[jmax] << "\n";
  }

  // just to show the use of running_window
  void test_running_window(datetime_interval<T> window) {
    std::size_t sum = 0;
    datetime_interval<T> from, to, vto;

    for (auto rw_it = m_x.rw_begin(window); !rw_it.hit_the_end(); ++rw_it) {
      auto first = rw_it.first(), centre = rw_it.centre();
      //     last   = rw_it.last();
      ...
      /*
         ...
         auto iqr = rw_it.iqr();
       */
      std::cout << "\n[" << rw_it.first().epoch().as_mjd() << " < "
                << rw_it.centre().epoch().as_mjd() << " < "
                << rw_it.vlast().epoch().as_mjd() << "]";
      std::cout << " [" << from.as_mjd() << ", " << vto.as_mjd() << ", "
                << to.as_mjd() << "]";
  ...
  std::ostream &dump(std::ostream &os, bool use_csv_format = false,
                     bool dates_as_ymd = false) const {
    char delim((use_csv_format) ? ',' : ' ');
    auto x_iter = m_x.cbegin(), y_iter = m_y.cbegin(), z_iter = m_z.cbegin();

    os << "epoch" << delim << "north" << delim << "sigma_north" << delim
       << "flag_north" << delim << "east" << delim << "sigma_east" << delim
       << "flag_east" << delim << "up" << delim << "sigma_up" << delim
       << "flag_up"
       << "\n";

    if (!dates_as_ymd) {
      for (; x_iter != m_x.cend(); ++x_iter, ++y_iter, ++z_iter) {
        os << x_iter.epoch().as_mjd() << delim << x_iter.data().value() << delim
           << x_iter.data().sigma() << delim << x_iter.data().flag() << delim
           << y_iter.data().value() << delim << y_iter.data().sigma() << delim
           << y_iter.data().flag() << delim << z_iter.data().value() << delim
           << z_iter.data().sigma() << delim << z_iter.data().flag() << "\n";
      }
    } else {
      std::vector<std::string> ds(m_epochs.size(), "yyyy-mm-dd hh:mm:ss");
      std::transform(m_epochs.cbegin(), m_epochs.cend(), ds.begin(),
                     [](const ngpt::datetime<T> &t) -> std::string {
                       return ngpt::strftime_ymd_hms(t);
                     });
      std::size_t i = 0;
      for (; x_iter != m_x.cend(); ++x_iter, ++y_iter, ++z_iter, ++i) {
        os << ds[i] << delim << x_iter.data().value() << delim
           << x_iter.data().sigma() << delim << x_iter.data().flag() << delim
           << y_iter.data().value() << delim << y_iter.data().sigma() << delim
           << y_iter.data().flag() << delim << z_iter.data().value() << delim
           << z_iter.data().sigma() << delim << z_iter.data().flag() << "\n";
  ...
  std::ostream &dump_model_line(std::ostream &os, const ts_model<T> &modelx,
                                const ts_model<T> &modely,
                                const ts_model<T> &modelz,
    ...
    if (!dates_as_ymd) {
      for (std::size_t i = 0; i < tvec.size(); ++i) {
        os << tvec[i].as_mjd() << delim << v1[i] << delim << v2[i] << delim
           << v3[i] << "\n";
      }
    } else {
  ...
  */
  ...
  const std::vector<epoch> *epoch_vector() const noexcept { return &m_epochs; }
  ...
  auto mean_coordinates() const noexcept {
