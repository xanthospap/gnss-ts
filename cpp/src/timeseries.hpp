...
public:
  ...
  /// Constructor.
  explicit data_point(double val = 0.0, double sigma = 1e-3,
                      tflag f = tflag{}) noexcept
  ...
  /// equality operator
  bool operator==(const data_point &other) noexcept {
...
/// @example  test_ts.cpp
///
template <class T, class F, typename = std::enable_if_t<T::is_of_sec_type>>
class timeseries {
public:
  ...
  /// \f$\widetilde{x}_{n+1}=\frac{x_{n+1}+n*\widetilde{x_n}}{n+1} =
  /// \widetilde{x_n}+\frac{x_{n+1}-\widetilde{x_n}}{n+1}\f$
  ...
  std::size_t ts2array(double *epoch_arr, double *val_arr, double &ave,
                       double &var) const {
    std::size_t N{this->data_pts() - this->skipped_pts()};
    double prev_ave{0e0};
    auto ts_start{this->cbegin()}, ts_stop{this->cend()};
    std::size_t index{0};
    double x, y;
  ...
  explicit timeseries(std::vector<epoch> *epochs = nullptr) noexcept
      : m_epochs(epochs), m_mean{0.0}, m_skipped{0} {
    if (m_epochs)
  ...
  explicit timeseries(std::size_t size_hint) noexcept
      : m_epochs(nullptr), m_mean{0.0}, m_skipped{0} {
  ...
  record &operator()(std::size_t i) {
    return std::tie((*m_epochs[i]), m_data[i]);
  }

  /// Get the epoch at a certain index (non-const)
  epoch &epoch_at(std::size_t i) { return m_epochs->operator[](i); }

  /// Get the epoch at a certain index (non-const)
  epoch epoch_at(std::size_t i) const { return m_epochs->operator[](i); }
  ...
  std::size_t epochs() const noexcept {
    return m_epochs ? m_epochs->size() : 0;
  }
  ...
  epoch last_epoch() const noexcept {
  ...
  timeseries(const timeseries &ts, std::size_t start = 0, std::size_t end = 0)
      : m_epochs(ts.m_epochs), m_mean{ts.m_mean}, m_skipped{ts.m_skipped} {
    if (start || end) {
      if (start && !end) {
        end = ts.m_data.size();
      }
  ...
  timeseries(timeseries &&ts) noexcept
      : m_epochs(ts.m_epochs), m_mean{std::move(ts.m_mean)},
        m_data{std::move(ts.m_data)}, m_skipped{std::move(ts.m_skipped)} {}
  ...
  double add_point(const entry &e)
  ...
  ///         skipped data points (if needed).
  /// @return The updated time-series mean value.
  double add_point(double val, double sigma = 1e-3, tflag f = tflag{}) {
  ...
  void mark(std::size_t index,
            F f) /* noexcept noop, m_data[index] could throw */
  {
  ...
  epoch central_epoch() const noexcept {
  ...
  /// last epochs.
  epoch central_valid_epoch() const noexcept {
  ...
  detrend(double& x0, double& vx, bool mark_outliers=false, double sigma0=1e-3)
  {
      ...
      // Compute the mean epoch as mjd; all deltatimes are computed as
  differences
      ...
      for (; it != it_end; ++it) {
          ...
          if ( !entry.skip() ) {
              ...
              model.assign_row(A, b, current_epoch, weight, dt, entry.value(),
  idx);
              ++idx;
          }
          ++counter;
      ...
      auto Q = (A.transpose() *
  A).fullPivLu().solve(Eigen::MatrixXd::Identity(N,N));
      std::cout<<"\n\tVar-Covar Info:";
  ...
  }; post_std_dev += residual*residual; res[i] = dp;
              ++idx;
          } else {
              res[i] = m_data[i];
          }
  ...
  auto
  qr_ls_solve(ngpt::ts_model<T> &model, double &post_std_dev,
              double sigma0 = 1e-03,
              bool set_model_epoch =
                  false, /* set or not the mean epoch for the (input) model */
    ...
    std::size_t idx{0},        // index (i.e. row of A and b matrices)
        counter{0};            // index of the current data point (m_data)
    datetime<T> current_epoch; // the current epoch
    timeseries_const_iterator<T, F> it = cbegin(), it_end = cend();
    ...
    for (std::size_t i = 0; i < epochs(); i++) {
      if (!m_data[i].skip()) {
        residual =
            u(idx) /
            (sigma0 /
             m_data[i].sigma()); /* is this correct, or should it be
                                    u(idx)/(sigma0^2/m_data[i].sigma()^2) */
        data_point<F> dp{residual, m_data[i].sigma(), m_data[i].flag()};
    ...
    post_std_dev = std::sqrt(post_std_dev) / (double)(idx - parameters);
    ...
    // for (int i=0; i<N; i++) {
    //    std::cout<<"\n\t"<<x(i)<<" +/- "<<post_std_dev*std::sqrt( Q(i,i) ) <<
    //    " ratio: "<<x(i) / (post_std_dev*std::sqrt( Q(i,i) ));
    // }
  ...
  /// (reagrdless of its flag).
  /// @todo is it noexcept?
  timeseries_const_iterator<T, F> cbegin() const {
  ...
  /// @todo is it noexcept?
  timeseries_const_iterator<T, F> cend() const {
  ...
  timeseries_const_iterator<T, F> upper_bound(const datetime<T> &t,
                                              std::size_t &idx) const noexcept {
    auto it = std::upper_bound(std::cbegin(*m_epochs), std::cend(*m_epochs), t);
    if (it == m_epochs->cend()) {
      idx = m_epochs->size();
      return this->cend();
    } else {
  ...
  ///                 element in the time-series with a time-stamp >= t; if not
  ///                 found, a const iterator to end.
  timeseries_const_iterator<T, F> lower_bound(const datetime<T> &t,
                                              std::size_t &idx) const noexcept {
    auto it = std::lower_bound(std::cbegin(*m_epochs), std::cend(*m_epochs), t);
    if (it == m_epochs->cend()) {
      idx = m_epochs->size();
      return this->cend();
    } else {
  ...
  running_window<T, F> rw_begin(const datetime_interval<T> &window) {
    running_window<T, F> it{*this, window};
    it.begin();
    return it;
  }

  running_window<T, F> rw_end(const datetime_interval<T> &window) {
    running_window<T, F> it{*this, window};
    it.end();
    return it;
  }
  ...
  std::ostream &dump(std::ostream &os) const {
    ...
    for (; iter != this->cend(); ++iter) {
      os << iter.epoch().as_mjd() << " " << iter.data().value() << " "
         << iter.data().sigma() << " " << iter.data().flag() << "\n";
    }
    return os;
  }
#endif
...
public:
  ...
  explicit timeseries_iterator(
      timeseries<T, F, typename std::enable_if_t<T::is_of_sec_type>> &ts)
      : m_timeseries{ts}, m_data_iter{ts.iter_start()},
  ...
  timeseries_iterator(const timeseries_iterator &it) noexcept
      : m_timeseries{it.m_timeseries}, m_data_iter{it.m_data_iter},
  ...
  bool operator==(const timeseries_iterator &it) const noexcept {
  ...
  int distance_from(const timeseries_iterator &it) const {
  ...
  timeseries<T, F> &timeseries_ref() noexcept { return m_timeseries; }
...
public:
    ...
    /// Constructor given a time-series instance.
    explicit
    timeseries_iterator(timeseries<T, F, typename
std::enable_if_t<T::is_of_sec_type>>& ts) : m_timeseries{ts}, m_idx{0} { assert(
ts.epoch_ptr() && ts.data_pts() == ts.epochs() ); }
    ...
    ///
    /// @todo    See the warning above; i really need to fix this some way.
Also,
    ...
    /// Return (a reference to) the time_series instance this iterator points
to. timeseries<T, F>& timeseries_ref() noexcept { return m_timeseries; }
...
/// class to alter the values of the timeseries (either the time or
/// data_points). The internals of this class are pretty simple; it just holds a
/// (reference to) time_series instance and two iterators, one for the epoch
/// vector and one for the data_point vector (both are of type std::vector).
///
/// @tparam T The time precision; this can be any class (of ggdatetime), for
///           which is_of_sec_type is true. This means that T could be e.g.
///           ngpt::seconds, ngpt::milliseconds, etc. The individual epochs
...
public:
  ...
  /// Constructor
  explicit timeseries_const_iterator(
      const timeseries<T, F, typename std::enable_if_t<T::is_of_sec_type>> &ts)
      : m_timeseries{ts}, m_data_iter{ts.citer_start()},
  ...
  /// Equality operator (i.e. both iterators point to the same element)
  bool operator==(const timeseries_const_iterator &it) const noexcept {
    return (m_data_iter == it.m_data_iter && m_epoch_iter == it.m_epoch_iter);
  }
  ...
  /// Return the distance (i.e. index difference) between two iterators.
  /// Obviously, they must belong to the same time-series.
  int distance_from(const timeseries_const_iterator &it) const {
    assert(&m_timeseries == &it.m_timeseries);
    return std::distance(it.m_data_iter, this->m_data_iter);
  }

  /// Return the time-delta between two iterators.
  interval_td delta_time(const timeseries_const_iterator &it) const {
    assert(&m_timeseries == &(it.m_timeseries));
    return m_epoch_iter->delta_date(*(it.m_epoch_iter));
  }

  /// Return the epoch the iterator instance is currently at.
  const epoch_td epoch() noexcept { return *m_epoch_iter; }

  /// Return the entry the iterator instance is currently at.
  const entry data() noexcept { return *m_data_iter; }
...
public:
  ...
  explicit running_window(timeseries<T, F> &ts, const interval_td &w)
      : m_window{w}, m_half_window{w}, m_iterator_begin{ts}, m_iterator{ts},
        m_iterator_end{ts}, m_END{ts.end()} {
  ...
  bool hit_the_end() const noexcept { return m_iterator == m_END; }
  ...
  bool operator==(const running_window &other) const noexcept {
  ...
  timeseries_iterator<T, F> &first() noexcept { return m_iterator_begin; }

  timeseries_iterator<T, F> &last() noexcept { return m_iterator_end; }

  timeseries_iterator<T, F> vlast() noexcept {
    auto it{m_iterator_end};
    return --it;
  }

  timeseries_iterator<T, F> &centre() noexcept { return m_iterator; }

  entry average() const noexcept {
  ...
  entry clean_average() const noexcept {
    double mean{0}, sigma{0};
    int size{0};
    for (auto it = m_iterator_begin; it != m_iterator_end; ++it) {
      if (!it.data().skip()) {
  ...
  entry median() const noexcept {
    ...
    if (size % 2) { /* odd size */
      ...
      std::nth_element(sigmas.begin(), sigmas.begin() + size / 2 + 1,
                       sigmas.end());
...
private:
  ...
  interval_td split_window() noexcept {
  ...
  interval_td m_window, m_half_window;
  timeseries_iterator<T, F> m_iterator_begin, m_iterator, m_iterator_end, m_END;
};

template <class T, class F>
void nikolaidis(timeseries<T, F> &residuals, timeseries<T, F> &original_ts,
  ...
  for (auto rw_it = residuals.rw_begin(window); !rw_it.hit_the_end(); ++rw_it) {
...
#ifdef DEBUG
  std::cout << "\n[DEBUG] Outliers: " << num_of_outliers << "/" << valid_pts
            << " = ";
