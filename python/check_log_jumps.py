...
def get_log_possible_jumps(logfn):
    ...
    ## date to datetime
    installed_at = datetime.datetime.combine(installed_at, datetime.datetime.min.time()) 
    ...
    for key, value in dct.items():
        ...
        if not installed_at.date() == t.date():
            posjumps.append({'t': t, 'comment': 'Receiver change ({:}'.format(os.path.basename(logfn))})
    ...
    for key, value in dct.items():
        ...
        if not installed_at.date() == t.date():
            ...
            for c,entry in enumerate(posjumps):
                if entry['t'] == t:
                    posjumps[c]['comment'] = 'Antenna/Receiver change ({:}'.format(os.path.basename(logfn))
                    isReceiverChange = True
            if not isReceiverChange:
                posjumps.append({'t': t, 'comment': 'Antenna change ({:}'.format(os.path.basename(logfn))})

    return posjumps

def check_jump(ts, jumpdct, component='east', max_data_points=30, min_data_points=15):
    if not ts.includes_key(component):
        errmsg = 'Error. Time-Series has no component named \'{:}\''.format(component)
        raise RuntimeError(errmsg)

    t = jumpdct['t']
    print('-> Checking possible jump at {:}'.format(t))
    tstart, tend = ts.time_span()
    if t <= tstart or t >= tend:
        errmsg = 'Error. Checking for jump for date outside Time-Series limits! t={:}'.format(t)
        raise RuntimeError(errmsg)
    ...
    if idx_end < min_data_points:
        errmsg = 'Error. Too few points to check possible jump at t={:}'.format(t)
        raise RuntimeError(errmsg)
    ...
    lmd = mdl.TsModel()
    x,res = lmd.fit(ts_cut.get('t'), ts_cut.get('east'))
    ...
    print('\tDeltay = {:}, Std = {:}'.format(abs(estimate_after - estimate_prior), slope_std))
