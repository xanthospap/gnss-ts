...
def dappend(dct, dentry):
    if t in dct:
        dct[t] = dentry if dentry['tstamp'] > dct[t]['tstamp']
    else:
        dct[t] = dentry
    return dct
...
def parse_cts(filename, **kwargs):
    ...
    if unique: ret = [d[key] for key in ret]

    ## sort based on epoch
    return ts.TimeSeries('', sorted(ret, key=lambda d return d['t']))
